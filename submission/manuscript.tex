...
rRNA gene sequences (7--10). Approaches for identifying ASVs do not
cluster sequences based on a distance-based threshold (11). Proponents
of ASVs are largely dissmissive of concerns that most bacterial genomes
have more than one copy of the \emph{rrn} operon and that those copies
are not identical (12, 13). Yet, ASVs and using too fine a threshold to
identify OTUs could split a single genome into multiple clusters.
Conversely, using too broad of a threshold to define OTUs could cluster
together multiple bacterial species into the same OTU. An example of
...
across species and for this analysis I measured the percentage of ASVs
and OTUs that contained 16S rRNA gene sequences from multiple species
...
respectively. Considering that species designations are inconsistently
applied and reflect multiple human-imposed biases, the risk of splitting
a genome into multiple OTUs is more problematic than clustering species
...
(v. 4.0.4). Within R, I used the tidyverse (v. 1.3.0), data.table (v.
1.13.2), Rcpp (v. 1.0.5), furrr (v. 0.2.1), here (v. 1.0.1) and
rmarkdown (v. 2.5) packages. The conception and development of this
