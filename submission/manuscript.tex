...
necessary to use a distance threshold of \textbf{5.25}\% to cluster full
...
each genome also increased. On average, there were \textbf{0.58}
variants per copy of the full length 16S rRNA gene and an average of
\textbf{0.32}, \textbf{0.25}, and \textbf{0.27} variants when
...
the \textbf{271} genome accessions of \textbf{\emph{Mycobacterium
tuberculosis}} in the \emph{rrn}DB each had \textbf{1} copy of the gene
per genome. However, across those accessions, there were \textbf{17}
...
length ASVs per genome (intraquartile range between \textbf{3} and
\textbf{6}). Across the \textbf{1,390} \emph{E. coli} genomes in the
\emph{rrn}DB, there were \textbf{1,402} versions of the gene. These
...
\textbf{5.25}\% was required to reduce full length ASVs into a single
OTU for 95\% of the species (Figure 2). Similarly, thresholds of
\textbf{5.25}, \textbf{2.50}, and \textbf{3.75}\% were required for the
...
\textbf{10}, \textbf{10}, \textbf{8}, and \textbf{10} copies of the
...
(\textbf{Figure 2}). Using ASVs, \textbf{4.11}\% of the ASVs contained
sequences from multiple species when considering full length sequences
and \textbf{10.87}, \textbf{16.20}, and \textbf{13.06}\% when
considering the V3-V4, V4, and V4-V5 regions, respectively. At the
commonly used 3\% threshold for defining OTUs, \textbf{27.41}\% of the
OTUs contained 16S rRNA gene sequences from multiple species when
considering full length sequences and \textbf{31.71}, \textbf{34.31},
and \textbf{34.78}\% when considering the V3-V4, V4, and V4-V5 regions,
respectively. Considering that species designations are unevenly applied
and reflect multiple human-imposed biases, the risk of splitting a
genome into multiple OTUs is more problematic than clustering species
together. Therefore, larger thresholds are advisable.
...
0.25 and 10.0\% in 0.25 percentage point increments were used to assign
sequences to OTUs.
...
between randomizations was less than \textbf{0.0024}. Because the range
